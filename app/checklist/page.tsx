...
// ensure runtime rendering (avoids SSG hiccups with useSearchParams)
...
function ChecklistRunPageInner() {
  ...
  // If missing cid, redirect to Assignments
  useEffect(() => {
    if (!cid) {
      const t = setTimeout(() => {
  ...
  const assignmentClass =
    assignedTo
      ? (assignedTo === meId
          ? "bg-[#E6F0FA] text-[#004C97]"
          : "bg-[#F7F9FC] text-[#1F2937]")
      : "bg-[#FFF4F4] text-[#DC3545]";
